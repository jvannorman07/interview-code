...
const { toConnectionString } = require('./dist/utils/db')
const { pgsql } = config.db
...
module.exports = {
   ...
   */
  connectionString: toConnectionString(pgsql, pgsql.root, pgsql.name),
   ...
   */
  shadowConnectionString: toConnectionString(pgsql, pgsql.root, `${pgsql.name}_shadow`),
   ...
   */
  rootConnectionString: toConnectionString(pgsql, pgsql.root, 'postgres'),
   ...
   */
  placeholders: {
    ':DATABASE_AUTHENTICATOR': pgsql.root.user,
    ':DATABASE_VISITOR': pgsql.visitor.user,
  },
  ...
  afterCurrent: [
    {
      _: 'command',
      command: 'SEED=true node seeds/seed.js',
    },
  ],
